...
async function assertCanPublish(appName, sender, provider) {
    ...
    if (repoAddress) {
        ...
        if (!isAllowed)
            throw new errors_1.AragonPluginError(`Account ${sender} does not have permissions to publish a new version in repo ${appName}`);
    }
    else {
        // If the repo does not exist yet, create a repo with the first version
        const { registryName } = (0, appName_1.getAppNameParts)(appName);
        ...
        if (!isAllowed)
            throw new errors_1.AragonPluginError(`Account ${sender} does not have permissions to create a new repo in registry ${registryName}`);
    }
}
