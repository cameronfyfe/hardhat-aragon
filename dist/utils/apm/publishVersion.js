...
async function publishVersion(appName, versionInfo, provider, options) {
    ...
    if (!semver_1.default.valid(version)) {
        throw new errors_1.AragonPluginError(`${version} is not a valid semantic version`);
    }
    const repoAddress = await provider.resolveName(appName);
    const versionArray = (0, utils_1.toApmVersionArray)(version);
    if (repoAddress) {
        ...
        const { shortName, registryName } = (0, appName_1.getAppNameParts)(appName);
        const registryAddress = await provider.resolveName(registryName);
        const managerAddress = options && options.managerAddress;
        if (!registryAddress)
            throw new errors_1.AragonPluginError(`Registry ${registryName} does not exist`);
        if (!managerAddress)
            throw new errors_1.AragonPluginError('managerAddress must be provided');
        return {
            to: registryAddress,
...
function encodePublishVersionTxData({ methodName, params, }) {
    switch (methodName) {
        ...
        default:
            throw new errors_1.AragonPluginError(`Unsupported publish version method name: ${methodName}`);
    }
}
