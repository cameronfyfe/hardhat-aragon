...
function toUtf8IfHex(hex) {
    return ethers_1.ethers.utils.isHexString(hex) ? ethers_1.ethers.utils.toUtf8String(hex) : hex;
}
...
function parseApmVersionReturn(res) {
    return {
        ...
        contentUri: ethers_1.ethers.utils.toUtf8String(res.contentURI),
    };
}
...
function toApmVersionArray(version) {
    ...
    if (!semverObj)
        throw new errors_1.AragonPluginError(`Invalid semver ${version}`);
...
function joinIpfsLocation(ipfsGateway, location) {
    return (0, url_1.urlJoin)(stipIpfsPrefix(ipfsGateway), 'ipfs', stipIpfsPrefix(location));
}
...
function contentUriToFetchUrl(contentUri, options) {
    if (!contentUri)
        throw new errors_1.AragonPluginError(`contentUri is empty`);
    const [protocol, location] = contentUri.split(/[/:](.+)/);
    switch (protocol) {
        case 'http':
        case 'https':
            if (!location)
                throw new errors_1.AragonPluginError(`contentUri location not set: ${contentUri}`);
            return location.includes('://') ? location : contentUri;
        case 'ipfs':
            if (!options || !options.ipfsGateway)
                throw new errors_1.AragonPluginError(`Must provide an ipfsGateway for protocol 'ipfs'`);
            return joinIpfsLocation(options.ipfsGateway, location);
        default:
            throw new errors_1.AragonPluginError(`Protocol '${protocol}' not supported`);
    }
}
...
function toContentUri(protocol, location) {
    if (!protocol)
        throw new errors_1.AragonPluginError('contentURI protocol must be defined');
    if (!location)
        throw new errors_1.AragonPluginError('contentURI location must be defined');
