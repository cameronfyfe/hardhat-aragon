...
async function guessGatewayUrl({ ipfsApiUrl, contentHash, ipfsGateway, }) {
    ...
    try {
        return await oneSuccess(urls.map(async (url) => {
            const testUrl = (0, url_1.urlJoin)(url, 'ipfs', contentHash);
            const res = await (0, node_fetch_1.default)(testUrl, { timeout: 3000 });
            // node-fetch does not throw on error status codes
            if (!res.ok)
                throw new errors_1.AragonPluginError(`Not ok ${res.statusText}`);
            await res.text();
...
function getPossibleGatewayUrls({ ipfsApiUrl, ipfsGateway, }) {
    const possibleUrls = [];
    const ipfsApiUrlParsed = (0, url_1.parseUrlSafe)(ipfsApiUrl);
    ...
    return (0, lodash_1.uniq)(possibleUrls).sort();
}
