 ...
 * or throws an AragonPluginError if the combination is not valid
...
function parseAndValidateBumpOrVersion(bumpOrVersion, prevVersion = '0.0.0') {
    if (!semver_1.default.valid(prevVersion))
        throw new errors_1.AragonPluginError('Previous version must be valid');
    if (bumps.includes(bumpOrVersion)) {
        ...
        if (!nextVersion)
            throw new errors_1.AragonPluginError(`Invalid bump ${bump}`);
        return {
            bump,
            nextVersion,
        };
        ...
        if (!isSemverClean(nextVersion))
            throw new errors_1.AragonPluginError(`next version must be a simple semver: ${nextVersion}`);
        ...
        throw new errors_1.AragonPluginError(`Invalid bump from ${prevVersion} to ${nextVersion}`);
    }
    else {
        // invalid case
        throw new errors_1.AragonPluginError(`Must provide a valid bump or valid semantic version: ${bumpOrVersion}`);
    }
}
